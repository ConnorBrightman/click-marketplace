...
export default async function VehicleDetailPage({ params }: PageProps) {
  const vehicle = await getVehicle(params.id).catch(() => notFound());
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <div className="flex items-center gap-3 mb-3">
                  {vehicle.dealer.logo_url ? (
                    ...
                    <div className="w-12 h-12 rounded-lg bg-navy-500 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                      <p className="font-bold text-gray-900 text-sm leading-tight">{vehicle.dealer.name}</p>
                      <span className="inline-flex items-center gap-1 bg-dealer-50 text-dealer-600 text-[10px] font-semibold px-1.5 py-0.5 rounded-full border border-dealer-200 whitespace-nowrap">
                    ...
                    {(vehicle.dealer.address_city || vehicle.dealer.address_postcode) && (
                      <p className="text-xs text-gray-400">
                        {[vehicle.dealer.address_city, vehicle.dealer.address_postcode].filter(Boolean).join(", ")}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {vehicle.dealer.phone && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 bg-dealer-400 hover:bg-dealer-500 text-white font-semibold text-xs px-3 py-2 rounded-lg transition-colors"
                    >
                  ...
                  {vehicle.dealer.slug && (
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 border border-gray-200 hover:border-teal-400 text-gray-600 hover:text-teal-600 font-semibold text-xs px-3 py-2 rounded-lg transition-colors"
                    >
                      View all stock →
                    </Link>
                  )}
